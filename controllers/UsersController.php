...
$router->get('/users', function () use ($twig, $auth, $authService, $mysqli) {
    ...
    $isAdmin = $isSuperAdmin || (int)$currentUser['role_id'] <= 2; // Super admin or admin
    $userModel = new UserModel($mysqli);
    $rolesManager = new RolesManager($mysqli);
    $unionModel = new UnionModel($mysqli);
    ...
    // Union filtering: admins can filter by any union, others are restricted to their own
    if ($isAdmin) {
        // Admin/Super Admin: filter by selected union if provided, otherwise see all
        if (!empty($unionId)) {
            $filters['union_id'] = (int)$unionId;
        }
    } elseif (!empty($currentUser['union_id'])) {
        // Non-admin roles: auto-restricted to their own union
    ...
    // Get unions list for admin filter dropdown
    $unions = [];
    if ($isAdmin) {
        $unions = $unionModel->getAllUnions();
    }
    ...
    echo $twig->render('users/admin_users_list.twig', [
        ...
        'filters'     => ['search' => $search, 'status' => $status, 'role' => $role, 'union_id' => $unionId],
        'roles'        => $roles,
        'unions'       => $unions,
        'is_admin'     => $isAdmin,
        'total_users'  => $total,
