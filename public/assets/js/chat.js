...
(function () {
  ...
  async function maybeSubscribePush() {
    ...
    try {
      ...
      // If the VAPID key has changed since the last subscription, unsubscribe
      // the old one and create a fresh subscription with the current key.
      // Without this, push delivery fails because the server signs with the
      // new private key but the browser's subscription was created with the
      // old public key.
      //
      // Also re-subscribe when there's no stored key but an existing
      // subscription exists (e.g. localStorage was cleared) — the old
      // subscription may be bound to a different key, so resubscribe to be safe.
      const storedVapidKey = localStorage.getItem(CONFIG.vapidKeyUsed) || '';
      if (sub && (!storedVapidKey || storedVapidKey !== data.data.public_key)) {
      ...
      if (!sub) {
        ...
        localStorage.setItem(CONFIG.vapidKeyUsed, data.data.public_key);
      }
