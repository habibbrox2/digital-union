...
(function () {
  ...
  // State
  // ========================
  const SEEN_MSG_MAX_SIZE = 500; // Cap to prevent memory leak
  let state = {
  ...
  // Helper: add to seenMessageIds with size cap to prevent memory leak
  function trackSeenMessage(id) {
    if (!id) return;
    state.seenMessageIds.add(id);
    // Evict oldest entries if over cap
    if (state.seenMessageIds.size > SEEN_MSG_MAX_SIZE) {
      const iter = state.seenMessageIds.values();
      for (let i = 0; i < 100; i++) {
        const val = iter.next().value;
        if (val !== undefined) state.seenMessageIds.delete(val);
      }
    }
  }
  ...
  async function fetchMessages() {
    ...
    // Calculate server time offset for accurate timeout calculations
  ...
  // Store server time offset for accurate timeout calculations
  ...
  function startSessionTimeoutMonitor() {
    ...
    state.timeoutTimer = setInterval(function () {
      ...
      // Use server time offset to avoid client clock drift issues
      const now = Date.now() + serverTimeOffset;
      const elapsed = Math.floor((now - new Date(state.lastVisitorActivityAt).getTime()) / 1000);
  ...
  // ========================
  // Firebase Cloud Messaging (FCM) — visitor browser notifications
  // ========================
  // The admin reply is delivered by the server (PushService/FCM) to every
  // subscribed browser, even when the chat tab is closed. This widget:
  //   1. Registers the FCM token once a session is established.
  //   2. Stores the {sessionId, sessionSig} binding for token refresh.
  //   3. Unsubscribes when the session is reset/expired.
  ...
  async function maybeSubscribePush() {
    ...
    try {
      ...
      if (typeof firebase === 'undefined' || typeof firebase.messaging === 'undefined') {
        console.log('[Chat] Firebase SDK not loaded, skipping FCM');
        return;
      }
      ...
      if (token) {
        ...
        await apiCall('POST', '/push/fcm-subscribe', {
          session_id: state.sessionId,
          session_sig: state.sessionSig,
          fcm_token: token
        });
        ...
        setInterval(async () => {
          try {
            ...
            console.error('[Chat] Token refresh check error:', err);
          }
        }, 6 * 60 * 60 * 1000);
      } else {
        console.log('[Chat] No FCM token available');
      }
    } catch (e) {
      console.error('[Chat] FCM subscription error:', e);
  ...
  function unsubscribePush(sessionId, sessionSig) {
    ...
    if (_fcmToken) {
      apiCall('POST', '/push/fcm-unsubscribe', {
        session_id: oldSessionId,
        session_sig: oldSessionSig || '',
        fcm_token: _fcmToken
      }).catch(function () {});

      // Try to delete the FCM token
