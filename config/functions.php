...
if (!function_exists('getFee')) {
    ...
    function showMessage($message, $type = 'success') {
        // 🔒 Sanitize $type to prevent class injection — whitelist allowed values
        $allowedTypes = ['success', 'error', 'warning', 'info', 'danger', 'primary', 'secondary'];
 ...
 * Prevents malformed URLs like "http://uploads/..." or "uploads/...".
 *
 * @param  string|null $path  Raw file path from upload or user input
 * @return string             Normalized relative path (e.g. "/uploads/application/foo.jpg")
 */
if (!function_exists('normalize_file_path')) {
    function normalize_file_path(?string $path): string
    {
        static $service = null;
        if ($service === null) $service = new SanitizationService();
        return $service->normalizeFilePath($path);
    }
}
