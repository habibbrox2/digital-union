...
if (!function_exists('normalizeDateToMysql')) {
    /**
     * Normalize a user-submitted date (e.g. DD-MM-YYYY) into MySQL DATE format (Y-m-d).
     * Returns '0000-00-00' for empty/invalid input so DATE columns never receive an invalid value.
     */
    function normalizeDateToMysql($date, $default = '0000-00-00') {
    ...
    function insertMembers($application_id, $certificate_type)
    {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            $memberData = [
                ...
                'birth_date' => normalizeDateToMysql(convertBanglaToEnglishNumber(si($births[$i] ?? ''))),
                'nid' => convertBanglaToEnglishNumber(si($nids[$i])),
                'gender' => null,
                'occupation' => null,
                'mobile' => null,
    ...
    function getFullApplicationData($application_id, $union_id = null, $includeMeta = false)
    {
        ...
        if ($includeMeta) {
            ...
            if (in_array($application['certificate_type'], ['warish', 'family'], true)) {
                ...
                $application['family_members'] = $application['warish_members'];
            }
        }
