...
class ApplicationManager
{
    ...
    public function renewLicense($application_id, $renewalData, $union_id = null)
    {
        ...
        try {
            ...
            // IMPORTANT: Include ALL fields that updateBusinessMeta() sets,
            // falling back to existing values to prevent data loss on renewal.
            // NOTE: For fee fields the ?? operator is NOT used because the
            // controller defaults empty POST fields to 0 (which is not null),
            // causing 0 to overwrite the existing value. Instead we use !empty()
            // so that only genuinely provided values replace the existing data.
            $updateBusinessData = [
                'business_name_en'    => $businessMeta['business_name_en'] ?? '',
                'business_name_bn'    => $businessMeta['business_name_bn'] ?? '',
                'vat_id'              => $businessMeta['vat_id'] ?? '',
                'tax_id'              => $businessMeta['tax_id'] ?? '',
                'paid_up_capital'     => $businessMeta['paid_up_capital'] ?? 0,
                'business_address_id' => $businessMeta['business_address_id'] ?? null,
                'license_fee'        => !empty($renewalData['license_fee']) ? (float)$renewalData['license_fee'] : ($businessMeta['license_fee'] ?? 0),
                'vat_amount'         => !empty($renewalData['vat_amount']) ? (float)$renewalData['vat_amount'] : ($businessMeta['vat_amount'] ?? 0),
                'occupation_tax'     => !empty($renewalData['occupation_tax']) ? (float)$renewalData['occupation_tax'] : ($businessMeta['occupation_tax'] ?? 0),
                'income_tax'         => !empty($renewalData['income_tax']) ? (float)$renewalData['income_tax'] : ($businessMeta['income_tax'] ?? 0),
                'signboard_tax'      => !empty($renewalData['signboard_tax']) ? (float)$renewalData['signboard_tax'] : ($businessMeta['signboard_tax'] ?? 0),
                'surcharge'          => !empty($renewalData['surcharge']) ? (float)$renewalData['surcharge'] : ($businessMeta['surcharge'] ?? 0),
                'total_fee'          => !empty($renewalData['total_fee']) ? (float)$renewalData['total_fee'] : ($businessMeta['total_fee'] ?? 0),
                'fiscal_year'        => $fiscalYear,
                'ownership_type_id'  => !empty($renewalData['ownership_type_id']) ? (int)$renewalData['ownership_type_id'] : ($businessMeta['ownership_type_id'] ?? null),
                'business_type_id'   => !empty($renewalData['business_type_id']) ? (int)$renewalData['business_type_id'] : ($businessMeta['business_type_id'] ?? null),
