...
class ExtraFields {
    ...
    /**
     * Get certificate types from term_translations
     */
    public function getCertificateTypes(): array
    {
        $result = $this->mysqli->query("SELECT slug, name_bn, name_en FROM term_translations WHERE is_certificate_type = 1 ORDER BY id");
        $types = [];
        if ($result) {
