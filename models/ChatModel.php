...
class ChatModel
{
    ...
    /**
     * Get email addresses of all admin-level users (superadmin + admin).
     *
     * @return array List of ['email' => ..., 'name' => ...] entries
     */
    public function getAdminEmails(): array
    {
        ...
        while ($row = $result->fetch_assoc()) {
            $emails[] = [
                'email' => $row['email'],
                'name' => $row['name_bn'] ?: $row['name_en'] ?: 'Admin',
            ];
        }
        $result->free();
        return $emails;
    ...
    public function insertMessage(string $sessionId, string $message, string $senderType, ?int $adminId = null, int $autoReply = 0): int
    {
        if ($adminId !== null) {
            ...
            $stmt->bind_param("ssssi", $sessionId, $message, $senderType, $adminId, $autoReply);
        } else {
            ...
            $stmt->bind_param("ssss", $sessionId, $message, $senderType, $autoReply);
        }
        $stmt->execute();
        $id = $stmt->insert_id;
        $stmt->close();
    ...
    /**
     * Save or update an FCM device token for a session.
     */
    public function saveFcmToken(string $sessionId, string $fcmToken, string $sessionSig = '', string $userType = 'visitor', ?int $userId = null): void
    {
        $stmt = $this->mysqli->prepare("
            INSERT INTO fcm_tokens (session_id, fcm_token, session_sig, user_type, user_id, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, NOW(), NOW())
            ON DUPLICATE KEY UPDATE
                session_id = VALUES(session_id),
                session_sig = VALUES(session_sig),
                user_type = VALUES(user_type),
                user_id = VALUES(user_id),
                updated_at = NOW()
        ");
        $stmt->bind_param("ssssi", $sessionId, $fcmToken, $sessionSig, $userType, $userId);
        $stmt->execute();
        $stmt->close();
    }

    /**
     * Get all FCM tokens for a chat session.
     */
    ...
    /**
     * Get all admin FCM tokens (for notifying admins of new visitor messages).
     */
     ...
     * Delete an FCM token.
     */
    public function deleteFcmToken(string $fcmToken): void
    {
        $stmt = $this->mysqli->prepare("DELETE FROM fcm_tokens WHERE fcm_token = ?");
    ...
    /**
     * Delete all FCM tokens for a session.
     */
    public function deleteFcmTokensBySession(string $sessionId): void
    {
        $stmt = $this->mysqli->prepare("DELETE FROM fcm_tokens WHERE session_id = ?");
        $stmt->bind_param("s", $sessionId);
        $stmt->execute();
        $stmt->close();
    }
    ...
    /**
     * Delete multiple invalid FCM tokens.
     */
    public function deleteInvalidFcmTokens(array $tokens): void
    {
        if (empty($tokens)) return;
        $placeholders = implode(',', array_fill(0, count($tokens), '?'));
        $stmt = $this->mysqli->prepare("DELETE FROM fcm_tokens WHERE fcm_token IN ($placeholders)");
    ...
    /**
     * Clean up expired FCM tokens (older than 30 days).
     */
    public function cleanExpiredFcmTokens(): int
    {
        $stmt = $this->mysqli->prepare("DELETE FROM fcm_tokens WHERE updated_at < DATE_SUB(NOW(), INTERVAL 30 DAY)");
        $stmt->execute();
