 ...
 *
 * This class provides confidentiality AND authenticity via an HMAC-SHA256
 * signature prepended to the ciphertext. Any tampering with the token (e.g.
 * flipping bytes to change the decrypted certificate type) is detected and
 * rejected during decryption, so the original plaintext cannot be altered.
 */
class CryptManager
{
    /**
    ...
    /**
     * @var int Length in bytes of the HMAC-SHA256 signature.
     */
    private $macLength = 32;

    /**
     * Encrypts the given plaintext data.
     *
     * Generates a random Initialization Vector (IV), encrypts the plaintext,
     * signs the IV+ciphertext with an HMAC-SHA256 (computed from the same key),
     * prepends the MAC to the payload, base64 encodes the result, and then
     * makes it URL-safe. The MAC guarantees the token cannot be tampered with.
     *
     * @param string $plaintext The data to be encrypted.
     * @return string The URL-safe base64 encoded authenticated token, or false on failure.
     */
    public function encrypt(string $plaintext): string|false
    {
        ...
        // Concatenate the IV and ciphertext, then authenticate the combined payload.
        $payload = $iv . $ciphertext;
        $mac = hash_hmac('sha256', $payload, $this->key, true);

        // Prepend the MAC so it travels with (and protects) the payload.
        $encrypted = base64_encode($mac . $payload);
    ...
    /**
     * Decrypts the given authenticated token.
     *
     * Verifies the HMAC-SHA256 signature first; if the MAC does not match,
     * the token is rejected (returns false) — this prevents any tampering
     * such as altering the decrypted certificate type. Only after a successful
     * verification is the ciphertext decrypted.
     *
     * @param string $token The URL-safe base64 encoded authenticated token.
     * @return string|false The original plaintext on success, or false on failure (e.g., invalid or tampered token).
     */
    public function decrypt(string $token): string|false
    {
        ...
        // Reject tokens that are too short to contain the MAC and a full IV.
        if (strlen($decoded) < $this->macLength + $iv_length) {
            return false; // Corrupted or invalid data
        }

        // Split the MAC from the (IV + ciphertext) payload.
        $mac = substr($decoded, 0, $this->macLength);
        $payload = substr($decoded, $this->macLength);

        // Constant-time MAC verification — reject any tampered token.
        $expectedMac = hash_hmac('sha256', $payload, $this->key, true);
