...
public function create(array $data): int
{
    ...
    public function searchAddresses(string $query): array {
        $like = '%' . $this->mysqli->real_escape_string($query) . '%';
        $stmt = $this->mysqli->prepare(
            "SELECT DISTINCT permanent_address_bn, permanent_address_en 
             FROM birth_applications 
             WHERE permanent_address_bn LIKE ? OR permanent_address_en LIKE ?
             LIMIT 10"
        );
        if (!$stmt) return [];
