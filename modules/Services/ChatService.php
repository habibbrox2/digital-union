...
class ChatService
{
    ...
    public static function handleFileUpload(array $file): array
    {
        ...
        return [
            'status' => 'success',
            'data' => [
                'file_url' => normalize_file_path('/uploads/chat/' . $safeName),
                'file_name' => $file['name'],
                'file_size' => $file['size'],
                'file_type' => $mimeType,
    ...
    /**
     * Send email notifications to admin users when no admin is online.
     *
     * @param string $visitorName  Visitor display name
     * @param string $message      Chat message (truncated)
    ...
    public function sendOfflineEmailNotification(string $visitorName, string $message, string $sessionId): void
    {
        try {
            ...
            $subject = "[{$visitorName}] নতুন চ্যাট বার্তা — অফলাইন নোটিফিকেশন";
...
<div style='max-width: 600px; margin: 0 auto; padding: 20px;'>
  <div style='background: #008B8B; color: white; padding: 15px 20px; border-radius: 8px 8px 0 0;'>
    <h2 style='margin: 0;'>💬 নতুন চ্যাট বার্তা</h2>
  </div>
  <div style='background: #f9f9f9; padding: 20px; border: 1px solid #ddd;'>
    <p><strong>দর্শক:</strong> {$visitorName}</p>
    <p><strong>সময়:</strong> {$timestamp}</p>
    <p><strong>বার্তা:</strong></p>
    <div style='background: white; padding: 12px; border-left: 4px solid #008B8B; margin: 10px 0;'>
      {$shortMessage}
    </div>
    <p style='margin-top: 20px;'>কোনো অ্যাডমিন বর্তমানে অনলাইনে নেই। দয়া করে উত্তর দিন।</p>
    <a href='{$chatUrl}' style='display: inline-block; background: #008B8B; color: white; padding: 10px 24px; text-decoration: none; border-radius: 5px; margin-top: 10px;'>চ্যাটে উত্তর দিন</a>
  </div>
  <div style='text-align: center; padding: 10px; color: #999; font-size: 12px;'>
    এই ইমেইলটি স্বয়ংক্রিয়ভাবে পাঠানো হয়েছে — লাইভ চ্যাট সিস্টেম থেকে
  </div>
</div>
</body>
</html>";
